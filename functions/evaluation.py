...
def show_result_MP(window_size, parameters, threshold, peaks):
    change_points = []
    for idx in range(np.shape(parameters)[0] - 1):
        pre = parameters[idx]
        suc = parameters[idx + 1]
        if pre != suc:
            ...
            change_points.append(0)
    precision, recall, f1 = metrics.print_f1(None, [threshold], change_points, window_size, False, peaks=peaks)
